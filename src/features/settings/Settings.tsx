...
import {
  selectAlarmAudioFiles,
  selectBreakAudioFiles,
  selectMusic,
...
export const Settings = () => {
  ...
  const closeModal = () => {
    ...
    updateConfigSettingsinDB(db, "alarmSound", {
      label: music.alarm.label,
      file: music.alarm.file,
    });
    updateConfigSettingsinDB(
      db,
      "alarmSoundVolume",
      Number(music.alarm.volume),
    );
    updateConfigSettingsinDB(db, "breakSound", {
      label: music.break.label,
      file: music.break.file,
    });
    updateConfigSettingsinDB(
      db,
      "breakSoundVolume",
      Number(music.break.volume),
    );
  ...
  const handleLongBreak = (event: ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    return value === ""
      ? dispatch(setLongBreakInterval(""))
      : dispatch(setLongBreakInterval(Math.min(30, Number.parseInt(value))));
  };
  ...
  const pickATheme = (themeName: string) => {
    const themeKey =
      themeContent.themeNumber === 1
        ? "focus"
        : themeContent.themeNumber === 2
          ? "short"
          : "long";
  ...
  return (
    <>
      <section
        className={`settings-modal backdrop-blur-sm ${toggle.settings ? "flex" : "hidden"} ${themeContent.show ? "invisible opacity-0" : "opacity-1 visible"} fixed inset-0 z-20 justify-center bg-black/75`}
        onClick={closeModal}
      >
        <section
          className={`${toggle.settings ? "flex" : "hidden"} relative top-11 m-3 h-modal-custom w-modal-custom max-w-xs flex-col rounded-md bg-slate-50 pt-4`}
        ...
        >
          ...
          <section className="modal-body overflow-auto px-4">
            {/* Timer options */}
            <section className="timer-options my-4 text-gray-500">
              <section className="flex items-center gap-1">
                ...
                <h2 className="text-sm font-medium uppercase">Timer</h2>
              </section>
              {/* Timer values */}
              <section className="timer-values mt-6">
                <header className="relative flex items-center gap-1">
                  ...
                  <section className="group">
                    ...
                    <p className="invisible absolute -top-14 left-0 w-full rounded bg-gray-500 p-2 text-xs text-white group-hover:visible">
                      Changing the fields below while timer is running will
                      reset the respective timer
                    </p>
                  </section>
                </header>
                <section className="mt-2 flex items-center justify-between">
                  <section className="flex flex-col items-center justify-center gap-2">
                    ...
                    <input
                      ...
                      name="focus"
                      id="focus"
                      className="rounded border-b-2 border-l-0 border-r-2 border-t-0 border-solid border-b-black/10 border-r-black/10 bg-gray-500/10 p-2 text-center text-sm text-black/50 shadow-number"
                      min={1}
                      max={999}
                      value={minutes?.focus}
                      onChange={(e) => handleChangeInMinutes(1, e)}
                      onKeyDown={handleKeyDown}
                      onBlur={(e) => removeLeadingZeroes(1, e)}
                    />
                  </section>
                  <section className="flex flex-col items-center justify-center gap-2">
                    ...
                    <input
                      ...
                      max={999}
                      value={minutes?.short}
                      onChange={(e) => handleChangeInMinutes(2, e)}
                      onKeyDown={handleKeyDown}
                      onBlur={(e) => removeLeadingZeroes(2, e)}
                    />
                  </section>
                  <section className="flex flex-col items-center justify-center gap-2">
                    ...
                    <input
                      ...
                      max={999}
                      value={minutes?.long}
                      onChange={(e) => handleChangeInMinutes(3, e)}
                      onKeyDown={handleKeyDown}
                      onBlur={(e) => removeLeadingZeroes(3, e)}
                    />
                  </section>
                </section>
              </section>
              ...
              <section className="my-4 flex items-center justify-between">
                ...
                <input
                  ...
                  name="auto-break"
                  id="auto-break"
                  className="relative h-8 w-14 cursor-pointer rounded-3xl bg-gray-500/10 shadow-checkbox transition-colors duration-500 ease-linear checked:bg-green-400"
                  aria-label="Set auto-focus to true"
                  checked={autoStart.break}
                  onChange={(e) =>
                    dispatch(
                      setAutoStart({
              ...
              <section className="mb-4 flex items-center justify-between">
                ...
                <input
                  ...
                  id="auto-focus"
                  className="relative h-8 w-14 cursor-pointer rounded-3xl bg-gray-500/10 shadow-checkbox transition-colors duration-500 ease-linear checked:bg-green-400"
                  aria-label="Set auto-break to true"
                  checked={autoStart.focus}
                  onChange={(e) =>
                    dispatch(
                      setAutoStart({
              ...
              <section className="flex items-center justify-between border-b border-black/20 pb-8 pt-2">
                <label
                ...
                <input
                  ...
                  className="w-20 rounded border-b-2 border-l-0 border-r-2 border-t-0 border-solid border-b-black/10 border-r-black/10 bg-gray-500/10 p-2 text-center text-sm text-black/50 shadow-number"
                  min={1}
                  max={999}
                  value={longBreakInterval}
                  onChange={(e) => handleLongBreak(e)}
                  onKeyDown={handleKeyDown}
                  onBlur={() =>
                    dispatch(
                      setLongBreakInterval(
                        longBreakInterval.toString().replace(/^0+/, "") || 4,
                      ),
                    )
                  }
                />
            ...
            {/* Theme options */}
            <section className="my-4 border-b border-b-black/20 text-gray-500">
              <section className="flex items-center gap-1">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                ...
                <h2 className="text-sm font-medium uppercase">Theme</h2>
              </section>
              <section className="theme-options mt-4">
                <section className="my-4 flex items-center justify-between">
                  ...
                  <section className="flex w-32 items-center justify-between">
                    <button
                      type="button"
                      className={`bg-${theme.focus} h-8 w-8 rounded-md`}
                      title="focus session background-color picker"
                      onClick={() => openThemeModal(1)}
                    ></button>
                    <button
                      type="button"
                      className={`bg-${theme.short} h-8 w-8 rounded-md`}
                      title="short break session background-color picker"
                      onClick={() => openThemeModal(2)}
                    ></button>
                    <button
                      type="button"
                      className={`bg-${theme.long} h-8 w-8 rounded-md`}
                      title="long break session background-color picker"
                      onClick={() => openThemeModal(3)}
                    ></button>
                  </section>
                </section>
                {/* Dark Mode */}
                <section className="my-4 flex items-center justify-between">
                  ...
                  <input
                    ...
                    id="dark-mode"
                    className="relative h-8 w-14 cursor-pointer rounded-3xl bg-gray-500/10 shadow-checkbox transition-colors duration-500 ease-linear checked:bg-green-400"
                    aria-label="Set dark mode to true"
                    checked={darkTheme}
                    onChange={(e) => dispatch(setDarkTheme(e.target.checked))}
                  />
                </section>
              </section>
            </section>
            ...
            <section className="my-4 border-b border-b-black/20 text-gray-500">
              <section className="flex items-center gap-1">
                ...
                <h2 className="text-sm font-medium uppercase">Sound</h2>
              </section>
              <section className="sound-options mt-4">
                <section className="my-4 flex items-baseline justify-between">
                  <p className="text-sm font-medium text-black/75">
                    Alarm Sound
                  </p>
              ...
              <section className="notifications-options mt-4">
                ...
                <section className="my-4 flex items-center justify-between">
                  ...
                  <input
                    ...
                    className="relative h-8 w-14 cursor-pointer rounded-3xl bg-gray-500/10 shadow-checkbox transition-colors duration-500 ease-linear checked:bg-green-400"
                    aria-label="Set silent-notifications to true"
                    checked={notificationOptions.silent}
                    onChange={(e) =>
                      dispatch(
                        setNotificationOptions({
                          silent: notificationOptions.enabled
                            ? e.target.checked
                            : false,
                        }),
                      )
            ...
            {/* Task options */}
            <section className="my-4 text-gray-500">
              <section className="flex items-center gap-1">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                ...
                <h2 className="text-sm font-medium uppercase">Tasks</h2>
              </section>
              <section className="task-options mt-4">
                <section className="my-4 flex items-center justify-between">
                  <p className="text-sm font-medium text-black/75">Add tasks</p>
                </section>
              </section>
            </section>
            {/* Tasks options */}
      ...
      {/* Theme Modal */}
      <section
        className={`theme-modal ${themeContent.show ? "flex" : "hidden"} fixed inset-0 z-20 items-center justify-center bg-black/75 px-4 backdrop-blur-sm`}
        onClick={() =>
          setThemeContent((previous) => ({ ...previous, show: false }))
        }
      >
        <section
          className={`theme-modal-content relative max-w-96 rounded-lg bg-white px-8 py-4`}
          onClick={(e) => e.stopPropagation()}
        >
          <h2 className="mb-4 border-b-2 border-b-gray-700/20 pb-4 text-center text-base font-medium text-gray-900/90">
            Pick a color for {themeContent.title}
          </h2>
          <section className="color-picker flex w-full flex-wrap items-start gap-4 overflow-auto">
            {themeContent.show &&
              colors.map(({ colorName, value }) => (
                <ThemeButton
                  ...
                  renderSVG={
                    themeContent.themeNumber === 1
                      ? theme.focus === colorName
                      : themeContent.themeNumber === 2
                        ? theme.short === colorName
                        : theme.long === colorName
                  }
                />
              ))}
          </section>
