...
export const Timer = () => {
  ...
  const updateSession = (skipped?: boolean) => {
    ...
    if (activeButton === 1) {
      const interval =
        typeof longBreakInterval === "number"
          ? longBreakInterval
  ...
  //     "keydown",
  //     spacebarFunction as unknown as EventListener,
  //   );
  //   return () => {
  ...
  return (
    <main
      className={`duration-250 my-6 flex w-full flex-col items-center justify-center transition-colors ease-linear ${isActive && darkTheme ? "bg-transparent" : "bg-white/10"} max-w-[480px] rounded-lg p-4 min-[500px]:mt-12 min-[500px]:p-6`}
    >
      <header
        className={`timer-header duration-250 flex items-center justify-center gap-4 transition-opacity ease-linear ${isActive && darkTheme ? "pointer-events-none opacity-0" : "pointer-events-auto opacity-100"}`}
      >
      ...
      <section className="timer pointer-events-none my-8 text-7xl min-[500px]:text-9xl min-[500px]:font-medium">
        {formatTime(countdown)}
      </section>
      <footer className="relative mb-4 flex w-full max-w-xs items-center justify-center">
        <button
          type="button"
          title="Press Spacebar to start/pause"
          id="start"
          className={`${activeButton === 1 ? `text-${theme.focus}` : activeButton === 2 ? `text-${theme.short}` : `text-${theme.long}`} rounded px-12 py-4 font-semibold uppercase ${isActive && darkTheme ? `bg-transparent text-white` : "bg-white"} w-full max-w-[65%] min-[500px]:text-xl ${isActive ? "translate-y-1 shadow-none" : "shadow-start"}`}
        ...
        <button
          type="button"
          title={`Skip ${activeButton === 1 ? "Focus" : "Break"} Session`}
          className={`absolute right-2 top-5 ml-8 ${isActive ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"} duration-200 ease-in`}
        >
          <svg
